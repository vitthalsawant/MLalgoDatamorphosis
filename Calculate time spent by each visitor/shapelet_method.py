...
This script processes video files to extract dwell time series and performs shapelet-based analysis.
"""
...
class PersonTracker:
    ...
    def update_trackers(self, frame, detections):
        ...
        # Use IoU-based tracking (no external tracker needed)
        ...
        for person_id in active_ids:
            ...
            if self.person_data[person_id]['is_visible']:
                ...
                if best_match_idx is None:
                    ...
                    if frames_since_seen > 10:  # Shorter threshold - person left frame
        ...
        for person_id in sorted(active_ids):
            ...
            # Find best matching detection
            best_match_idx = None
            best_iou = 0.5  # Increased from 0.3 to 0.5 for better matching
            ...
            if best_match_idx is not None:
                # Update existing person (visible in this frame)
        ...
        # Create new trackers for unmatched detections (new people entering)
        for idx, det_bbox in enumerate(detections):
...
class DwellTimeAnalysisGUI:
    ...
    def start_analysis(self):
        ...
        if not self.video_path or not os.path.exists(self.video_path):
            tk.messagebox.showerror("Error", "Please select a valid video file first.")
            return
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] in ['--gui', '-g', '--window']:
            use_gui = True
        elif sys.argv[1] in ['--cli', '-c', '--command']:
            use_gui = False
        else:
            # First argument is a file path, use CLI mode
            use_gui = False
    ...
    # CLI mode
    print("=" * 60)
    print("VIDEO VISITOR DWELL TIME ANALYSIS")
    print("=" * 60)
    
    # Get video file path
    if len(sys.argv) > 1 and sys.argv[1] not in ['--gui', '-g', '--window', '--cli', '-c', '--command']:
    ...
    # Ask user what analysis they want
    print("\nSelect analysis type:")
    print("1. Track visitors/people and calculate time spent by each")
    print("2. Shapelet-based dwell time analysis (original method)")
    ...
    try:
        if analysis_type == "1":
            ...
            print("\n" + "=" * 60)
            print("VISITOR TRACKING AND TIME ANALYSIS")
            print("=" * 60)
            ...
            if len(sys.argv) > 3:
                ...
                response = input("\nSave annotated video with tracking? (y/n, default=y): ").strip().lower()
                if response != 'n':
                    os.makedirs("output", exist_ok=True)
                    output_video_path = os.path.join("output", "tracked_visitors.mp4")
                    save_video = True
...
if __name__ == "__main__":
    # By default, launch GUI if available
    if HAS_TKINTER and len(sys.argv) == 1:
        # No arguments, launch GUI
        app = DwellTimeAnalysisGUI()
        app.run()
    else:
        # Arguments provided or no GUI, use CLI
        main()
